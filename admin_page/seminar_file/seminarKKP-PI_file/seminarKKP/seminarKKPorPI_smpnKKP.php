...
error_reporting(E_ALL ^ (E_NOTICE));
  ...
  if($cek=='nope'){
    ...
    $mail->Subject = 'Status Pendaftaran Seminar KKP';
    $mail->Body=
...
-$_SESSION[level] Sistem Informasi";
  ...
  echo "<script>window.alert('Data berhasil dikonfirmasi dan Email berhasil dikirim!')
   window.location='../admin_page/index2.php?ke=kkP_dftr'</script>";
 }
/*-------------------------------------------*/
...
/*Bagian Email Pendaftaran Diterima tanpa perubahan Dosen */
   ...
   $mail->Subject = 'Status Pendaftaran Seminar KKP';
   $mail->Body=
"Pendaftaran Seminar KKP anda telah diterima dengan detail sebagai berikut:
...
/*-------------------------------------------------- */
  ...
  $qry=$link->query("UPDATE tbldaftarseminar_kkporpi SET
    ...
    $mail->Subject = 'Status Pendaftaran Seminar KKP';
    $mail->Body=
"Pendaftaran Seminar KKP Anda telah diterima dengan penggantian pada dosen penguji yang anda usulkan, yaitu:
