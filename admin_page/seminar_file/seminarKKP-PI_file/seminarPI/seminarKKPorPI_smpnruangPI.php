...
error_reporting(E_ALL ^ (E_NOTICE));
  ...
  if($konf=='tdK'){
   ...
   unlink($loc.$syarat);
    ...
    $mail->Subject = 'Status Pendaftaran Tanggal Seminar PI';
    $mail->Body=
"Pendaftaran Tanggal dan Waktu Seminar PI anda tidak dapat diterima dikarenakan adanya persyaratan yang kurang yaitu:
...
-$_SESSION[level] Sistem Informasi";
  ...
  $mail->send();
   ...
   window.location='../admin_page/index2.php?ke=P1_ruang'</script>";
   /*jika persyaratan lengkap */
        ...
        if(empty($nim) || empty($kode) || empty($ruang) || empty($konf)){
         ...
         if(empty($ruang)){
           echo "<script>window.alert('Data Kosong!')
           window.location='index2.php?ke=P1_konfruang&kode=$kode'</script>";
         }else{
  ...
  /*Delete file persyaratan jika sudah konfirmasi*/
    ...
    $mail->addAddress("$email");
    $mail->Subject = 'Status Ruangan Seminar PI';
    $mail->Body=
"Pengajuan Tanggal dan waktu seminar PI anda telah diterima. Berikut adalah detail pendaftaran seminar anda:
...
-$_SESSION[level] Sistem Informasi";
  ...
  window.location='../admin_page/index2.php?ke=P1_jdwl'</script>";
 }
  }
}
}
