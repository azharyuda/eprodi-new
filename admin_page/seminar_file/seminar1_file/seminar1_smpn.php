...
error_reporting(E_ALL ^ (E_NOTICE));
  ...
  $qry=$link->query("SELECT dosbing1, dosbing2, dosen_penguji1 FROM tblmahasiswa WHERE NIM='$NIM'");
  ...
  if(empty($konf)){
    ...
    window.location='index2.php?ke=sempr0_konf&kd=$kode'</script>";
  }
  if($konf=="tdK"){
...
$loc='../admin_page/files_upload/seminar_file/seminar1_Persyaratan/untuk_sekprodi/';
unlink($loc.$syarat);
    $qry=$link->query("DELETE FROM tblseminar_proposal WHERE kode_daftar='$kode'") or die(mysqli_error());

    $mail->addAddress("$email");
    $mail->Subject = 'Status Pendaftaran Tanggal Seminar Proposal';
    $mail->Body=
"Pendaftaran Tanggal dan Waktu Seminar Proposal anda tidak dapat diterima dikarenakan adanya persyaratan yang kurang yaitu:
  ...
  window.location='index2.php?ke=home'</script>";
    ...
    if(empty($ruang)){
      ...
      if(mysqli_num_rows($cekruangan)>0){
        ...
        ruangan='$ruang',
        hasil_seminarproposal='$stat'
        WHERE kode_daftar='$kode'") or die(mysqli_error());
        echo "<script>window.alert('Data Berhasil Disimpan')
        window.location='index2.php?ke=sempr0_jdwl'</script>";
      ...
      $loc2='../admin_page/files_upload/seminar_file/seminar1_Persyaratan/untuk_kaprodi/';
      unlink($loc2.$syarat2);

      $mail->addAddress("$email");
      $mail->Subject = 'Status Ruangan Seminar Proposal';
      $mail->Body=
"Pengajuan Tanggal dan waktu seminar Proposal anda telah diterima. Berikut adalah detail pendaftaran seminar anda:
...
-$_SESSION[level] Sistem Informasi";
          ...
          window.location='../admin_page/index2.php?ke=sempr0_jdwl'</script>";
        }
      }
    }
  }
