...
error_reporting(E_ALL ^ (E_NOTICE));
          ...
          if(empty($cek)){
           ...
           window.location='index2.php?ke=semh4s_konfdaftar&kd=$kd'</script>";
          }elseif($cek=='nope'){
...
unlink($loc.$sarat);
             ...
             $mail->Subject = 'Status Pendaftaran Seminar Hasil';
             $mail->Body=
"Pendaftaran Seminar Hasil anda tidak dapat diterima dikarenakan adanya persyaratan yang kurang yaitu:
           ...
           window.location='index2.php?ke=home'</script>";
            ...
            if(empty($pilihan)){
             ...
             if(empty($dosenubah) || empty($kd) ||  empty($nim)){
              ...
              $qry=$link->query("UPDATE tbldosen SET kuota_penguji2=kuota_penguji2 - 1 where NIDN='$nidn'");
              $qry2=$link->query("UPDATE tblmahasiswa SET
              dosen_penguji2='$dosenubah' WHERE NIM='$nim'") or die(mysqli_error());

             $mail->addAddress("$email");
             $mail->Subject = 'Status Pendaftaran Seminar Hasil';
             $mail->Body=
"Pendaftaran Seminar Hasil anda telah diterima dengan detail sebagai berikut:
...
-Dosen Penguji 2           : $dosenubah

Penggantian dosen penguji dikarenakan: $alasan

Demikian pemberitahuan status pendaftaran seminar hasil Anda. Silahkan segera mengajukan tanggal dan waktu seminar.
