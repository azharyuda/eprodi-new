<?php
  ...
  $jadizip=move_uploaded_file($tmp_name, '../admin_page/files_upload/seminar_file/seminar1_Persyaratan/untuk_sekprodi/'.$name);
  if($jadizip){

    $zip = new ZipArchive;
    $namezip="$NIM-persyaratan-$tgl_input.zip";
    if($zip->open('../admin_page/files_upload/seminar_file/seminar1_Persyaratan/untuk_sekprodi/'.$namezip, ZipArchive::CREATE) !== TRUE) {
        echo "gagal zip";
      }
       $zip->addFile('../admin_page/files_upload/seminar_file/seminar1_Persyaratan/untuk_sekprodi/'.$name, $name);
       $zip->close();
     }
   }
 }
...
$qry=$link->query("UPDATE tbldaftarseminar_skripsi SET persyaratan2='$namezip' WHERE kode_daftar='$kode'");
 ...
 $qry=$link->query("INSERT INTO tblseminar_proposal VALUES(
   '$kode',
   '$NIM',
   '$tglsem',
   '$jam',
  ...
  window.location='../user_page/index.php?ke=home'</script>";
   ...
   $hapus=glob('../admin_page/files_upload/seminar_file/seminar1_Persyaratan/untuk_sekprodi/*.jpg');
